...
import ComboOffer from '@/app/Components/pages/ProductPages/ComboOffer';
import NoOfferProduct from '@/app/Components/pages/ProductPages/NoOfferProduct';
...
export default function ProductPage() {
    ...
    if(param.productSlug.split("-")[0]==="skinprod"){
      ...
      if(param.productSlug.split("-")[0]==="combo"){
        FinalProducts = COMBO;
      }
      else{
  ...
  const [thumbsSwiper, setThumbsSwiper] = useState(null);
    ...
    const cartStore = useCartData();

    const handleCart = (productID) => {


      let selectedProduct = cartStore.cartproducts.find((item)=>{
       return productID===item.key
      })
        if(selectedProduct){
         let modifiedCartData =  cartStore.cartproducts.map((item)=>{
          ...
          cartStore.SetCartProducts(modifiedCartData);
        }else{
          let CartInfo = [...cartStore.cartproducts,{...product,quantity:quantity,key:productID,prodDesc:cartStore.productName,price:cartStore.productPrice}];
          cartStore.SetCartProducts(CartInfo);
        }
    }
  ...
  return (
    <>
       ...
       <div className="lg:container-xsm">
      <div className="grid grid-cols-12" >
         <div className="lg:col-span-6 col-span-12 md:px-20 px-10">
        <Swiper
        style={{
      ...
      >
        ...
        {product.carousel_images.map((item,index)=>{
          return(
            <SwiperSlide key={index}>
            <Image alt='image' src={item} className='rounded-md' />
          </SwiperSlide>
          )
        })}
      </Swiper>
      ...
      >
        {product.carousel_images.map((item,index)=>{
         ...
         <div className="lg:col-span-6 col-span-12 md:px-0 px-10">
          ...
          {/* Offer Calculator */}
          {
            product.offer? product.offer.offerType==="pack"?<ProductOffer price={product.price} title={product.title} productType={product.productType} product_img_1={product.carousel_images[product.carousel_images.length-1]} images={product.carousel_images} offerProduct={product.offer.offerProduct} />:<ComplementaryOffer prodId={param.productSlug} price={product.price} title={product.title} productType={product.productType} product_img_1={product.carousel_images[product.carousel_images.length-1]} size={product.size} images={product.carousel_images} offerProduct={product.offer.offerProduct} />:product.combo?<ComboOffer discountedPrice={product.discountedprice} price={product.price} title={product.title} />:<>
             <NoOfferProduct price={product.price} title={product.title} />
            </>
          }
          {/* Quantity calculator */}
           ...
           <div className="flex w-full">
            ...
            <button class="btn w-full mt-6 py-2.5 font-semibold text-lg rounded-[60px]" data-hs-overlay="#hs-overlay-example" onClick={()=>{
              handleCart(`${product.key}-${cartStore.productName.toLowerCase()}`)
            }} >Add to cart</button>
            </div>
           </div>
           {/* Offer Zone */}
